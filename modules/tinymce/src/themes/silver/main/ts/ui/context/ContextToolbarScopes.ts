...
const categorise = (
  ...
  navigate: (type: string, destForm: InlineContent.ContextForm | InlineContent.ContextToolbarGroup) => void
): ScopedToolbars => {
  ...
  const registerGroup = (key: string, toolbarSpec: InlineContent.ContextToolbarGroupSpec) => {
    ...
    groupNavigators[key] = {
      ...
      type: 'button', // Here, type should be 'contexttoolbargroupbutton'?
      chevron: 'right',
      // type: 'contexttoolbargroupbutton' as any,
      onAction: () => {
        navigate(key, contextGroup);
      }
    };
